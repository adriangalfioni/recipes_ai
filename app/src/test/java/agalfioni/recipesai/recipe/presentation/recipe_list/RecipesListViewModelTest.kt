...
import agalfioni.recipesai.recipe.presentation.recipe_list.utils.IA_GENERATION_TIMEOUT_MILLIS
...
class RecipesListViewModelTest {
    ...
    @Test
    fun `init calls usecase and updates state with success`() = runTest {
        val recipes = listOf(
            Recipe(
                id = "1",
    ...
    @Test
    fun `init updates state with error on failure`() = runTest {
    ...
    @Test
    fun `timeout updates state with deadline exceeded error`() = runTest {
    ...
    @Test
    fun `onRetry event resets state and calls usecase again`() = runTest {
    ...
    @Test
    fun `initial state before init coroutine runs`() = runTest {
        val recipes = listOf(
            Recipe(
                id = "1",
                title = "Spaghetti Bolognese",
    ...
    @Test
    fun `Use case parameter validation`() = runTest {
    ...
    @Test
    fun `Multiple retry clicks results in only one successful state update`() = runTest {
        var successCount = 0
        coEvery { useCase(any(), any()) } coAnswers {
            delay(1000)
            successCount++
    ...
    @Test
    fun `ViewModelScope cancellation cancels pending generation`() = runTest {
