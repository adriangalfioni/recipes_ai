package agalfioni.recipesai.ingredientsdetector.data.utils

import agalfioni.recipesai.ingredientsdetector.data.FakeLoggingImageObserver
...
class ImageProcessorImplTest {
    ...
    @Test
    fun `compress large image reduces size`() =
        runTest {
            val context = ApplicationProvider.getApplicationContext<Context>()
    ...
    private fun getFileSize(uriString: String): Long = getFileFromUri(uriString).length()
...
private fun Long.toMBString(): String = "${String.format("%.2f", this / 1024.0)} MB"
