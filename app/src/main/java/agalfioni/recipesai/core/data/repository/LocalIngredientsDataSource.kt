...
class LocalIngredientsDataSource(
    ...
    private var cachedRawIngredients: List<LocalizedIngredient>? = null
    ...
    ): List<LocalizedIngredient> = withContext(Dispatchers.IO) {
    ...
    private suspend fun getOrLoadRawData(): List<LocalizedIngredient> {
        mutex.withLock {
            ...
            val gson = Gson()
            val listType = object : TypeToken<List<LocalizedIngredient>>() {}.type
            val loadedList: List<LocalizedIngredient> = gson.fromJson(jsonString, listType)
